-- ═══════════════════════════════════════════════════════════════════
-- CRM ALIUN — Migration v1.1 (Conversion Leads ready)
-- Crea: crm_leads, crm_activities, crm_deals, crm_capi_logs
-- + crm_pipeline_stats() RPC
-- + meta_lead_id para Conversion Leads API
-- + crm_capi_logs para auditoría de eventos Meta
...
CREATE TABLE IF NOT EXISTS public.crm_leads (
  ...
  meta_lead_id    VARCHAR(20),              -- Meta Lead ID (15-17 dígitos) para Conversion Leads API
...
CREATE INDEX IF NOT EXISTS idx_crm_leads_meta_lead_id ON public.crm_leads(meta_lead_id);
...
-- ─── TABLA 4: crm_capi_logs ───────────────────────────────────
-- Auditoría de eventos enviados a Meta Conversion Leads API
CREATE TABLE IF NOT EXISTS public.crm_capi_logs (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  lead_id         uuid REFERENCES public.crm_leads(id) ON DELETE SET NULL,
  meta_lead_id    VARCHAR(20),               -- Meta Lead ID enviado
  event_name      text NOT NULL,             -- ej: "Initial Lead from Facebook"
  stage_crm       text NOT NULL,             -- stage en CRM que disparó el evento
  payload_sent    jsonb,                     -- JSON completo enviado a Meta
  meta_response   jsonb,                     -- respuesta de Meta API
  status          text NOT NULL DEFAULT 'sent'
                  CHECK (status IN ('sent','received','failed','rejected')),
  event_time      timestamptz NOT NULL DEFAULT now(),
  created_at      timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_crm_capi_logs_lead ON public.crm_capi_logs(lead_id);
CREATE INDEX IF NOT EXISTS idx_crm_capi_logs_status ON public.crm_capi_logs(status);
CREATE INDEX IF NOT EXISTS idx_crm_capi_logs_event ON public.crm_capi_logs(event_name);
CREATE INDEX IF NOT EXISTS idx_crm_capi_logs_time ON public.crm_capi_logs(event_time DESC);
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'capi_events_sent', (
      SELECT count(*)::int FROM public.crm_capi_logs WHERE status = 'sent'
    ),
    'capi_events_failed', (
      SELECT count(*)::int FROM public.crm_capi_logs WHERE status IN ('failed','rejected')
    ),
...
ALTER TABLE public.crm_capi_logs ENABLE ROW LEVEL SECURITY;
